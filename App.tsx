...
// --- SUB-VIEWS ---
// We reuse the exact same view components, ensuring they receive the full props
// Re-defining them here to ensure the file is self-contained and functional
  ...
  return (
    <div className="space-y-6 md:space-y-8 animate-fade-in pb-24 w-full px-2 md:px-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-5 2xl:grid-cols-7 min-[1800px]:grid-cols-8 gap-5 md:gap-6">
        {activeEvents.length === 0 && (<div className="col-span-full flex flex-col items-center justify-center py-24 md:py-32 bg-white/30 rounded-[3rem] border border-dashed border-slate-300 w-full"><Layout size={48} md:size={64} className="text-slate-300 mb-4 md:mb-6" strokeWidth={1}/><p className="text-slate-500 font-semibold text-lg md:text-xl">Nenhum evento ativo.</p></div>)}
        {activeEvents.map(event => (
          <div key={event.id} className="group glass-panel rounded-[2rem] md:rounded-[2.5rem] overflow-hidden flex flex-col h-80 md:h-88 bg-white/50 relative cursor-default hover:shadow-xl hover:shadow-slate-200 transition-all duration-500">
            <div className="relative h-40 md:h-44 bg-slate-100 overflow-hidden shrink-0">{event.imageUrl ? (<img src={event.imageUrl} title={event.title} className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110" />) : (<div className="absolute inset-0 bg-gradient-to-br from-indigo-50 to-slate-100" />)}<div className="absolute top-4 left-4 z-10">{event.status === 'PROSPECT' ? (<span className="px-3 py-1.5 rounded-xl bg-amber-100/95 text-amber-800 text-[10px] font-bold uppercase tracking-wider shadow-sm backdrop-blur-md border border-white/20">Prospect</span>) : (<span className="px-3 py-1.5 rounded-xl bg-emerald-100/95 text-emerald-800 text-[10px] font-bold uppercase tracking-wider shadow-sm backdrop-blur-md border border-white/20">Ativo</span>)}</div><div className="absolute top-4 right-4 flex gap-2 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-300 z-20 pointer-events-auto transform translate-y-0 md:-translate-y-2 md:group-hover:translate-y-0">{event.websiteUrl && (<a href={event.websiteUrl} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="p-2 bg-white/90 hover:bg-blue-50 text-slate-600 hover:text-blue-600 backdrop-blur rounded-xl transition shadow-md cursor-pointer" title="Visitar Site"><ExternalLink size={16} strokeWidth={2.5} /></a>)}<button onClick={(e) => { e.stopPropagation(); onEditEvent(event); }} className="p-2 bg-white/90 hover:bg-indigo-50 text-slate-600 hover:text-indigo-600 backdrop-blur rounded-xl transition shadow-md cursor-pointer" title="Editar"><Edit2 size={16} strokeWidth={2.5}/></button><button onClick={(e) => { e.stopPropagation(); onCompleteEvent(event.id); }} className="p-2 bg-white/90 hover:bg-emerald-50 text-slate-600 hover:text-emerald-600 backdrop-blur rounded-xl transition shadow-md cursor-pointer" title="Finalizar e Arquivar"><CheckCircle2 size={16} strokeWidth={2.5}/></button><button onClick={(e) => { e.stopPropagation(); onDeleteEvent(event.id); }} className="p-2 bg-white/90 hover:bg-rose-50 text-slate-600 hover:text-rose-600 backdrop-blur rounded-xl transition shadow-md cursor-pointer" title="Excluir"><Trash2 size={16} strokeWidth={2.5}/></button></div></div>
            <div className="p-5 md:p-6 flex flex-col flex-1 justify-between relative"><div className="absolute -top-7 right-6 w-14 h-14 bg-white rounded-2xl shadow-lg flex items-center justify-center text-slate-400 text-xs font-bold border border-slate-50 z-10 transform rotate-3 group-hover:rotate-0 transition-transform duration-300"><div className="flex flex-col items-center leading-none gap-1"><span className="text-[10px] uppercase tracking-widest">{formatDate(event.date).split(' ')[1]}</span><span className="text-xl text-indigo-600">{formatDate(event.date).split(' ')[0]}</span></div></div><div className="mt-4"><h3 className="text-lg font-bold text-slate-800 leading-tight mb-2 truncate" title={event.title}>{event.title}</h3><p className="text-xs text-slate-500 truncate max-w-[160px] flex items-center gap-1.5 font-medium"><MapPin size={14} className="text-indigo-400"/> {event.location || 'Local não definido'}</p></div><div className="flex items-end justify-end"><div className="w-10 h-10 rounded-full bg-slate-50 border border-slate-100 flex items-center justify-center shadow-sm group-hover:bg-indigo-600 group-hover:border-indigo-500 transition-all duration-300 text-slate-300 group-hover:text-white group-hover:scale-110"><ChevronRight size={20} strokeWidth={2.5} /></div></div></div>
          </div>
        ))}
      </div>
    </div>
      ...
      <div className="flex flex-col xl:flex-row gap-8 animate-fade-in w-full min-h-full pb-20 px-2 md:px-0">
        <div className="flex-1 flex flex-col">
          ...
          <div className="glass-panel rounded-[2.5rem] p-4 md:p-8 border-white flex flex-col min-h-[500px] md:min-h-[600px] shadow-lg w-full bg-white/50 overflow-x-auto">
            <div className="grid grid-cols-7 mb-6 text-indigo-500 text-[10px] md:text-xs font-extrabold text-center uppercase tracking-widest min-w-[300px]"><div>Dom</div><div>Seg</div><div>Ter</div><div>Qua</div><div>Qui</div><div>Sex</div><div>Sab</div></div>
            <div className="grid grid-cols-7 auto-rows-fr gap-1 md:gap-3 flex-1 min-w-[300px]">
              {Array.from({ length: firstDayOfMonth }).map((_, i) => <div key={`empty-${i}`} />)}
              {Array.from({ length: daysInMonth }).map((_, i) => {
                const day = i + 1; const dayDemands = getDemandsForDay(day); const isToday = day === today.getDate() && currentMonth === today.getMonth() && currentYear === today.getFullYear(); const isSelected = selectedDay === day;
                return (
                  <div key={day} onClick={() => setSelectedDay(day)} className={`rounded-xl md:rounded-2xl p-1 md:p-2 border-2 transition-all cursor-pointer relative flex flex-col items-center justify-start group min-h-[60px] md:min-h-[100px] ${isSelected ? 'border-indigo-400 bg-indigo-50 shadow-inner' : 'border-transparent bg-white/40 hover:bg-white/70 hover:shadow-md'} ${isToday ? 'text-indigo-600' : 'text-slate-600'}`}>
                    ...
                    <div className="flex gap-1 mt-2 justify-center w-full flex-wrap content-start px-1">{dayDemands.slice(0, 4).map((d, idx) => (<div key={idx} className={`w-1.5 h-1.5 rounded-full ${d.priority === Priority.HIGH ? 'bg-rose-500' : d.priority === Priority.MEDIUM ? 'bg-amber-400' : 'bg-emerald-400'} shadow-sm`} />))}{dayDemands.length > 4 && <div className="w-1 h-1 rounded-full bg-slate-300" />}</div>
                  </div>
                );
              })}
            </div>
        ...
        <div className="w-full xl:w-[400px] glass-panel rounded-[2.5rem] p-6 md:p-8 flex flex-col h-auto border-white bg-white/60 shadow-xl md:sticky md:top-24">
          <h3 className="text-lg font-bold text-slate-700 mb-5 border-b border-slate-200/60 pb-4 flex items-center gap-3"><Clock size={22} className="text-indigo-500" strokeWidth={2} />{selectedDay ? `${selectedDay} de ${monthNames[currentMonth]}` : 'Detalhes'}</h3>
          <div className="flex-1 overflow-y-auto custom-scrollbar space-y-4 max-h-[300px] xl:max-h-[600px] pr-2">
            {selectedDay ? (getDemandsForDay(selectedDay).length > 0 ? (getDemandsForDay(selectedDay).map(d => (
                  <div key={d.id} className={`group relative p-5 rounded-2xl border transition hover:scale-[1.02] ${d.status === Status.DONE ? 'bg-slate-50/50 border-slate-100 opacity-60' : 'bg-white/80 border-white shadow-sm'}`}>
                    <div className="pr-8"><p className="text-slate-800 font-bold text-sm leading-tight">{d.title}</p>{d.description && (<p className="text-xs text-slate-500 mt-1 line-clamp-2 leading-relaxed">{d.description}</p>)}</div>
                    <div className="flex items-center justify-between mt-3"><span className={`text-[10px] px-2.5 py-1 rounded-lg font-bold uppercase tracking-wider ${d.priority === Priority.HIGH ? 'bg-rose-100 text-rose-600' : d.priority === Priority.MEDIUM ? 'bg-amber-100 text-amber-600' : 'bg-emerald-100 text-emerald-600'}`}>{d.priority}</span><span className="text-xs text-slate-400 font-medium">{d.status}</span></div>
                    <div className="absolute top-4 right-4 flex gap-2"><button onClick={(e) => { e.stopPropagation(); onCompleteDemand(d.id); }} className="p-2 text-slate-300 hover:text-emerald-500 rounded-xl transition" title="Concluir e Arquivar"><CheckCircle2 size={18} strokeWidth={2} /></button><button onClick={(e) => { e.stopPropagation(); onDeleteDemand(d.id); }} className="p-2 text-slate-300 hover:text-rose-500 rounded-xl transition" title="Excluir Demanda"><Trash2 size={18} strokeWidth={2} /></button></div>
                  </div>
                ))) : <div className="flex flex-col items-center justify-center h-40 md:h-64 text-slate-400 text-sm italic"><CalendarIcon size={48} className="mb-4 opacity-20" strokeWidth={1}/><p>Nenhuma demanda para este dia.</p></div>) : (<p className="text-slate-400 text-sm text-center mt-10">Selecione um dia para ver as demandas.</p>)}
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-20 w-full px-2 md:px-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
         {notes.length === 0 ? (<div className="col-span-full p-20 text-center flex flex-col items-center justify-center bg-white/10 rounded-[2.5rem] border border-dashed border-slate-200"><StickyNote size={56} className="text-indigo-200 mb-5" strokeWidth={1} /><p className="text-slate-500 font-medium text-lg">Nenhuma nota criada.</p><button onClick={onAdd} className="text-indigo-600 text-sm font-bold mt-2 hover:underline">Criar primeira nota</button></div>) : (
...
const App: React.FC = () => {
  ...
  // --- REALTIME SUBSCRIPTION (Granular Handling) ---
  useEffect(() => {
    if (!supabase) return;
    ...
    const channel = supabase.channel('db_changes')
        .on('postgres_changes', { event: '*', schema: 'public', table: 'events' }, (payload) => handlePayload(payload, setEvents, mapEvent))
        .on('postgres_changes', { event: '*', schema: 'public', table: 'demands' }, (payload) => handlePayload(payload, setDemands, mapDemand))
        .on('postgres_changes', { event: '*', schema: 'public', table: 'clients' }, (payload) => handlePayload(payload, setClients, mapClient))
        .on('postgres_changes', { event: '*', schema: 'public', table: 'notes' }, (payload) => handlePayload(payload, setNotes, mapNote))
        .subscribe();
  ...
  const handleSaveClient = async () => {
      ...
      if(supabase) {
          const payload = { id, name: clientForm.name, company: clientForm.company, role: clientForm.role, email: clientForm.email, phone: clientForm.phone, status: clientForm.status||'Potential', notes: clientForm.notes };
          if(clientForm.id) await supabase.from('clients').update(payload).eq('id', id); else await supabase.from('clients').insert([payload]);
      }
  };
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (file && file.size < 5000000) {
        const reader = new FileReader(); reader.onloadend = () => { if (typeof reader.result === 'string') setEventForm(p => ({ ...p, imageUrl: reader.result as string })); }; reader.readAsDataURL(file);
      } else if(file) showToast("Arquivo muito grande.", 'error');
  };
  ...
  return (
    <div className="min-h-screen flex bg-transparent font-sans selection:bg-indigo-100 selection:text-indigo-800">
      ...
      <aside className={`fixed lg:sticky top-0 h-screen w-72 flex-shrink-0 border-r border-white/60 bg-white/40 glass-sidebar flex flex-col justify-between z-50 transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
        <div>
          <div className="h-20 lg:h-24 flex items-center px-6 lg:px-8 border-b border-white/40"><div className="w-10 h-10 lg:w-11 lg:h-11 bg-gradient-to-tr from-indigo-600 to-blue-500 rounded-2xl flex items-center justify-center shadow-lg shadow-indigo-200 shrink-0 text-white"><Layout size={20} strokeWidth={2.5} /></div><div className="ml-4"><h1 className="font-bold text-lg tracking-tight text-slate-800">Lon Demandas</h1><div className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500 shadow-sm"></span><p className="text-[10px] text-slate-500 uppercase tracking-wider font-bold">Pro v6.0 Realtime</p></div></div><button onClick={() => setIsMobileMenuOpen(false)} className="lg:hidden ml-auto p-2 text-slate-400"><X size={24} /></button></div>
